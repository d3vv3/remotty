...
describe("SessionRow status presentation", () => {
  it.each([
    ...
    expect(container.querySelector("button")?.getAttribute("aria-label")).toBe(`Review, ${label}, /test`)
    expect(container.querySelector(".session-row-folder")?.getAttribute("title")).toBe("/test")
    expect(container.querySelector(".session-row-folder")?.textContent).toBe("test")
