...
// Relative timestamps and delivery badges can refresh without parsing unchanged prose.
const Markdown = memo(function Markdown({ text }: { text: string }) {
  return <ReactMarkdown remarkPlugins={[remarkGfm]} components={{ a: ({ children, href }) => <a href={href} target="_blank" rel="noreferrer">{children}</a> }}>{text}</ReactMarkdown>
})
  ...
  return <article className={`message ${message.info.role}`} aria-label={isUser ? "Your message" : `${author} response`}>
    ...
    <div className="message-body">
        {message.parts.map((part, index) => part.type === "text" && part.text
          ? isUser ? <p key={index}>{part.text}</p> : <div className="markdown" key={index}><Markdown text={part.text} /></div>
