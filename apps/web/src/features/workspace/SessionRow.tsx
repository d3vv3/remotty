...
import { relativeTime } from "./workspaceModel"
...
export function SessionRow({ session, needsInput, offline, selected, onSelect }: { session: SessionSummary; needsInput: boolean; offline: boolean; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <button className={`session-row ${selected ? "selected" : ""} ${state === "needs-input" ? "attention" : ""}`} aria-label={`${session.title}, ${stateLabel}`} aria-current={selected ? "true" : undefined} onClick={onSelect}>
